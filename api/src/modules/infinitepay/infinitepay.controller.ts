...
export class InfinitePayController {
  ...
  async checkout(@Body() dto: CriarPedidoDto, @Req() req: { user?: AuthUser }) {
    if (!req.user || req.user.tipo !== 'visitor') {
      throw new BadRequestException('Checkout online disponível apenas para o cliente.');
    }
  ...
  async confirmar(@Body() dto: ConfirmarCheckoutDto, @Req() req: { user?: AuthUser }) {
    if (!req.user || req.user.tipo !== 'visitor') {
      throw new BadRequestException('Confirmação de checkout disponível apenas para o cliente.');
    }
  ...
  async webhook(@Body() body: Record<string, unknown>, @Res({ passthrough: false }) res: Response) {
